...
class SingleTask(nn.Module):
    ...
    def __init__(self, num_classes, classify=False):
...
def v4(num_classes, classify, pretrained='imagenet'):
    if pretrained:
        settings = pretrained_settings['inceptionv4'][pretrained]
        # both 'imagenet'&'imagenet+background' are loaded from same parameters
        model = SingleTask(num_classes, classify=classify)
        # model.load_state_dict(model_zoo.load_url(settings['url']))
        load_pre_model_dict(model, model_zoo.load_url(settings['url'])) #2018.09.21
